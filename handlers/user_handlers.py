...
def push_user_handlers(dp: Dispatcher, user_dict: dict):
    ...
    async def process_start_command(message: Message):
        ...
        if user_id not in user_dict:
            user_dict.setdefault(user_id, {'total_games': 0,
                                           'total_wins': 0,
                                           'nobody_wins': 0})
    ...
    async def get_stat(message: Message):
        ...
        await message.answer(
            text=f'Всего игр: {user_dict[user_id]["total_games"]}\n'
                 f'Количество побед: {user_dict[user_id]["total_wins"]}\n'
                 f'Ничьих: {user_dict[user_id]["nobody_wins"]}')

    # Этот хэндлер срабатывает на согласие пользователя играть в игру
    @dp.message_handler(Text(equals=[LEXICON_RU['yes_button']],
                             ignore_case=True))
    ...
    @dp.message_handler(Text(equals=[LEXICON_RU['no_button']],
                             ignore_case=True))
    async def process_no_answer(message: Message):
        await message.answer(text=LEXICON_RU['no'])

    # Этот хэндлер срабатывает на любую из игровых кнопок
    @dp.message_handler(Text(equals=[LEXICON_RU['rock'],
                                     LEXICON_RU['paper'],
                                     LEXICON_RU['scissors']],
                             ignore_case=True))
